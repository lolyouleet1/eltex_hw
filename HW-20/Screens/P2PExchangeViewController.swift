...
final class P2PExchangeViewController: UIViewController {
    ...
    // MARK: - Coordinator
    weak var coordinator: P2PExchangeViewControllerCoordinator?
    
    // MARK: - Lifecycle
    init(viewModel: P2PExchangeViewModel, logger: AppLoggerProtocol = AppLogger.p2pTrading) {
    ...
    override func viewDidLoad() {
        ...
        logger.info(Constants.screenOpenedMessage)
        viewModel.start()
    }
}
...
private extension P2PExchangeViewController {
    @objc func handleCurrencyTapped(_ gesture: UITapGestureRecognizer) {
        guard let tappedLabel = gesture.view as? UILabel else {
            logger.warning(
                Constants.currencySelectionRequestRejectedMessage,
                metadata: [
                    AppLogMetadataKey.reason: Constants.invalidCurrencyControlReason
                ]
            )
            return
        }
        ...
        logger.info(
            Constants.currencySelectionRequestedMessage,
            metadata: [
                AppLogMetadataKey.side: selectionSide.logDescription
            ]
        )
        coordinator?.p2pExchangeViewController(
            self,
            didRequestCurrencySelectionFor: selectionSide
        )
    }
    
    @objc func handleWalletButtonTapped() {
        logger.info(Constants.walletNavigationRequestedMessage)
        coordinator?.p2pExchangeViewControllerDidRequestWallet(self)
    }
}
...
private extension P2PExchangeViewController {
    func showExchangeAlert(for index: Int) {
        guard let inputViewModel = viewModel.exchangeInputViewModel(for: index) else {
            logger.warning(
                Constants.exchangeAlertRejectedMessage,
                metadata: [
                    AppLogMetadataKey.reason: Constants.missingExchangeInputReason
                ]
            )
            return
        }
        ...
        let executeAction = UIAlertAction(title: inputViewModel.actionTitle, style: .default) { [weak self, weak alert] _ in
            ...
            self?.logger.info(Constants.exchangeConfirmationRequestedMessage)
        ...
        logger.info(Constants.exchangeAlertPresentedMessage)
    ...
    func showAlert(_ viewModel: P2PAlertViewModel) {
        ...
        logger.info(Constants.alertPresentedMessage)
        present(alert, animated: true)
    }
}
...
extension P2PExchangeViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(
            ...
            logger.error(
                Constants.offerCellPreparationFailedMessage,
                metadata: [
                    AppLogMetadataKey.reason: Constants.invalidCellTypeReason
                ]
            )
...
// MARK: - P2POfferCellDelegate
extension P2PExchangeViewController: P2POfferCellDelegate {
    func p2pOfferCellDidTapSellerInfo(_ cell: P2POfferCell) {
        guard let indexPath = tableView.indexPath(for: cell) else {
            logger.warning(
                Constants.sellerInfoRequestRejectedMessage,
                metadata: [
                    AppLogMetadataKey.reason: Constants.missingIndexPathReason
                ]
            )
            return
        }
        
        logger.info(Constants.sellerInfoRequestedMessage)
        coordinator?.p2pExchangeViewController(
            self,
            didRequestSellerInfoFor: indexPath.row
        )
    }
}
...
private extension P2PExchangeViewController {
    enum Constants {
        ...
        static let screenOpenedMessage = "The P2P exchange screen was opened."
        static let currencySelectionRequestedMessage = "P2P currency selection was requested."
        static let currencySelectionRequestRejectedMessage = "P2P currency selection request was rejected."
        static let walletNavigationRequestedMessage = "P2P wallet navigation was requested."
        static let exchangeAlertPresentedMessage = "P2P exchange input alert was presented."
        static let exchangeAlertRejectedMessage = "P2P exchange input alert presentation was rejected."
        static let exchangeConfirmationRequestedMessage = "P2P exchange confirmation was requested."
        static let alertPresentedMessage = "P2P result alert was presented."
        static let offerCellPreparationFailedMessage = "P2P offer cell preparation failed."
        static let sellerInfoRequestedMessage = "P2P seller information was requested."
        static let sellerInfoRequestRejectedMessage = "P2P seller information request was rejected."
        static let invalidCurrencyControlReason = "invalidCurrencyControl"
        static let missingExchangeInputReason = "missingExchangeInput"
        static let invalidCellTypeReason = "invalidCellType"
        static let missingIndexPathReason = "missingIndexPath"
