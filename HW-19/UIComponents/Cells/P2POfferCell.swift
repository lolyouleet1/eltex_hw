...
protocol P2POfferCellDelegate: AnyObject {
...
final class P2POfferCell: UITableViewCell {
    ...
    private let sellerInfoButton = UIButton(type: .system)
    
    // MARK: - Delegate
    weak var delegate: P2POfferCellDelegate?
...
private extension P2POfferCell {
    ...
    func setupLabels() {
        ...
        sellerLabel.numberOfLines = Constants.labelNumberOfLines
        
        rateLabel.font = .systemFont(ofSize: Constants.detailFontSize, weight: .medium)
        rateLabel.textColor = Constants.positiveTextColor
        rateLabel.numberOfLines = Constants.labelNumberOfLines
        
        reserveLabel.font = .systemFont(ofSize: Constants.detailFontSize)
        reserveLabel.textColor = Constants.secondaryTextColor
        reserveLabel.numberOfLines = Constants.labelNumberOfLines
        
        labelsStackView.axis = .vertical
        labelsStackView.spacing = Constants.stackSpacing
    }
    
    func setupSellerInfoButton() {
        sellerInfoButton.setImage(
            UIImage(systemName: Constants.sellerInfoButtonImageName),
            for: .normal
        )
        sellerInfoButton.tintColor = Constants.primaryColor
        sellerInfoButton.addTarget(
            self,
            action: #selector(handleSellerInfoButtonTapped),
            for: .touchUpInside
        )
    }
    ...
    func setupHierarchy() {
        ...
        containerView.addSubview(sellerInfoButton)
...
private extension P2POfferCell {
    func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            labelsStackView.trailingAnchor.constraint(lessThanOrEqualTo: sellerInfoButton.leadingAnchor, constant: -Constants.infoButtonSpacing),
            labelsStackView.bottomAnchor.constraint(equalTo: containerView.bottomAnchor, constant: -Constants.contentInset),
            
            sellerInfoButton.trailingAnchor.constraint(equalTo: containerView.trailingAnchor, constant: -Constants.contentInset),
            sellerInfoButton.centerYAnchor.constraint(equalTo: containerView.centerYAnchor),
            sellerInfoButton.widthAnchor.constraint(equalToConstant: Constants.infoButtonSize),
            sellerInfoButton.heightAnchor.constraint(equalToConstant: Constants.infoButtonSize)
        ])
    }
}
...
// MARK: - Actions
private extension P2POfferCell {
    @objc func handleSellerInfoButtonTapped() {
        delegate?.p2pOfferCellDidTapSellerInfo(self)
    }
}
...
private extension P2POfferCell {
    enum Constants {
        static let sellerInfoButtonImageName = "info.circle"
        ...
        static let infoButtonSpacing: CGFloat = 12
        static let infoButtonSize: CGFloat = 28
