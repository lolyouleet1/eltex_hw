...
private extension AuthViewController {
    ...
    func setupActions() {
        loginTextField.addTarget(
            self,
            action: #selector(handleLoginEditingChanged),
            for: .editingChanged
        )
        passwordTextField.addTarget(
            self,
            action: #selector(handlePasswordEditingChanged),
            for: .editingChanged
        )
...
// MARK: - Actions
private extension AuthViewController {
    @objc func handleLoginEditingChanged() {
        viewModel.handleLoginChange(loginTextField.text)
    }
    
    @objc func handlePasswordEditingChanged() {
        viewModel.handlePasswordChange(passwordTextField.text)
    }
