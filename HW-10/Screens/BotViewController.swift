...
private extension BotViewController {
    ...
    func setupStackView() {
        balanceLabel.text = "Balance: \(session.stock.balance)"
...
private extension BotViewController {
    ...
    @objc func handleStartBotButtonTapped() {
        ...
        let (finalBalance, finalProfit) = session.stock.getFinalResult(cycles)
        ...
        session.operations = session.stock.getOperations(cycles)
        operations = session.operations
        session.candlesticks = CandlestickFactory.makeCandlesticks(from: session.operations)
    }
