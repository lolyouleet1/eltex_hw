...
final class GraphViewModel {
    ...
    // MARK: - Dependencies
    private var candlestickItems: [GraphCandlestickItemViewModel]
    private var pointItems: [LineChartPointViewModel]
    ...
    func update(tradingResult: TradingRunResult?) {
        candlestickItems = Self.makeCandlestickItems(
            from: tradingResult?.candlesticks ?? [],
            preserving: candlestickItems
        )
        pointItems = Self.makeLineItems(from: tradingResult?.lineChartPoints ?? [])
...
private extension GraphViewModel {
    ...
    static func makeCandlestickItems(
        ...
        result.reserveCapacity(candlesticks.count)
        
        let preservedCount = min(candlesticks.count, previousItems.count)
        
        for index in 0..<preservedCount {
            let previousItem = previousItems[index]
            result.append(
                GraphCandlestickItemViewModel(
                    candlestick: candlesticks[index],
                    bodyHeight: previousItem.bodyHeight,
                    tailHeight: previousItem.tailHeight,
                    verticalOffset: previousItem.verticalOffset
                )
            )
        }
        
        var currentOffset = result.last?.verticalOffset ?? CGFloat.random(
            in: Constants.minimumVerticalOffset...Constants.maximumVerticalOffset
        )
        
        for candlestick in candlesticks.dropFirst(preservedCount) {
            currentOffset += CGFloat.random(
                in: -Constants.verticalOffsetStep...Constants.verticalOffsetStep
            )
            currentOffset = min(
